...
impl LithophaneConfig {
    ...
    pub fn validate(&self) -> crate::error::Result<()> {
        if self.color_pixel_width <= 0.0 {
            return Err(crate::error::PixestlError::Config("color_pixel_width must be positive".to_string()));
        }
        if self.texture_pixel_width <= 0.0 {
            return Err(crate::error::PixestlError::Config("texture_pixel_width must be positive".to_string()));
        }
        if self.color_pixel_layer_thickness <= 0.0 {
            return Err(crate::error::PixestlError::Config("color_pixel_layer_thickness must be positive".to_string()));
        }
        if self.color_pixel_layer_number == 0 {
            return Err(crate::error::PixestlError::Config("color_pixel_layer_number must be positive".to_string()));
        }
        if self.texture_min_thickness <= 0.0 {
            return Err(crate::error::PixestlError::Config("texture_min_thickness must be positive".to_string()));
        }
        if self.texture_max_thickness <= self.texture_min_thickness {
            return Err(crate::error::PixestlError::Config("texture_max_thickness must be greater than texture_min_thickness".to_string()));
        }
        if self.plate_thickness < 0.0 {
            return Err(crate::error::PixestlError::Config("plate_thickness must be non-negative".to_string()));
        }
        if !self.color_layer && !self.texture_layer {
            return Err(crate::error::PixestlError::Config("At least one of color_layer or texture_layer must be enabled".to_string()));
        }
        Ok(())
    }
